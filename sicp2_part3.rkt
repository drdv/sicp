...
(module Section/2.4.1/rectangular sicp
  (#%provide real-part
             imag-part
             magnitude
             angle
  ...
  (module+ test
    (#%require rackunit)
    (display "--> Section/2.4.1/rectangular\n")
...
(module Section/2.4.1/polar sicp
  (#%provide real-part
             imag-part
             magnitude
             angle
...
(module Section/2.4.3 sicp
  (#%require (only racket/base module+ hash-set! hash-ref make-hash)
             (only (submod "sicp1.rkt" common-utils) square tolerance)
             (only (submod ".." Section/2.4.2) attach-tag))

  #|
  Implementations of put and get are not given so in order to test the code I use a
  built-in hash-table to implement double displatch with (op . type) as a key.
  |#
...
(module+ test
  (require (submod ".." Section/2.4.1/rectangular test)
           (submod ".." Section/2.4.1/polar test)
